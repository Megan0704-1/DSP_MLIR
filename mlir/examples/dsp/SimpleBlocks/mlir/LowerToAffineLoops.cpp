...
//===----------------------------------------------------------------------===//
// ToyToAffine RewritePatterns: IntegerConstant operations
//===----------------------------------------------------------------------===//

// FIXME: integer tensor is not yet support
// to support it, add literal integer parsing logic in parser
struct IntegerConstantOpLowering : public OpRewritePattern<dsp::IntegerConstantOp> {
  using OpRewritePattern<dsp::IntegerConstantOp>::OpRewritePattern;

  LogicalResult matchAndRewrite(dsp::IntegerConstantOp op,
                                PatternRewriter &rewriter) const final {
    DenseIntElementsAttr intConstVal = op.getValue();
    Location loc = op.getLoc();
    ...
    if (!valueShape.empty()) {
      ...
      // FIXME: current impl of integer constant should only be using this condition.
      constantIndices.push_back(
          rewriter.create<arith::ConstantIndexOp>(loc, 0));
    }
    ...
    // generating a store when the recursion hits the base case.
    SmallVector<Value, 2> indices;
    auto valueIt = intConstVal.value_begin<IntegerAttr>();
    std::function<void(uint64_t)> storeElements = [&](uint64_t dimension) {
      // The last dimension is the base case of the recursion, at this point
      // we store the element at the given index.
      if (dimension == valueShape.size()) {
...
void ToyToAffineLoweringPass::runOnOperation() {
  ...
  patterns.add<AddOpLowering, ConstantOpLowering, /*test*/ IntegerConstantOpLowering, FuncOpLowering, MulOpLowering, 
